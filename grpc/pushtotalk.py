...
from custom_command import command, play_media, pause_media, init_player, start_ga, end_ga
    ...
    def assist(self):
        ...
        for resp in self.assistant.Assist(iter_log_assist_requests(),
            ...
            if resp.speech_results:
                speech_result = ' '.join(r.transcript for r in resp.speech_results)
                logging.info('Transcript of user request: "%s".', speech_result)

                # 録音終了後の最終結果
                if(stop_recording):
                    # 独自コマンドチェック
                    if command(speech_result):
                        # 終了処理
